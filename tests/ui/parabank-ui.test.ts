...
test.describe('Parabank E2E Tests', () => {
  test('Complete Parar-Banking Workflow - Registration to Bill Payment', async ({
    ...
    if (accounts.length >= 2) {
      ...
      Logger.info(
        `Funds transferred: ${transferAmount} from ${accounts[0].accountId} to ${accounts[1].accountId}`
      );
    }
    ...
    Logger.info(
      `Bill payment completed: ${billPayment.amount} to ${billPayment.payeeName}`
    );
    ...
    const transactions = await parabankApi.findTransactionsByAmount(
      accounts[1].accountId,
      transferAmount
    );
    ...
    const billPaymentTransaction = transactions.find(
      t => Math.abs(t.amount - transferAmount) < 0.01 // Handle floating point precision
    );
    expect(billPaymentTransaction).toBeDefined();
    await parabankApi.validateTransactionDetails(
