...
test.describe('API Tests - Transactions', () => {
  test('Find Transactions by Amount - Independent API Test', async ({ 
    ...
    if (accounts.length >= 2) {
      ...
      Logger.info(`Test transaction created: ${testAmount} from ${accounts[0].accountId} to ${accounts[1].accountId}`);
    }
    ...
    const transactions = await api.findTransactionsByAmount(accounts[1].accountId, testAmount);
    ...
    const targetTransaction = transactions.find(t => 
      Math.abs(t.amount - testAmount) < 0.01 // Handle floating point precision
    );
  ...
  test('Find Transactions by Amount - Error Handling', async ({ 
    ...
    const transactions = await api.findTransactionsByAmount(nonExistentAccountId, amount);
    // Should return empty array for non-existent account
    expect(Array.isArray(transactions)).toBeTruthy();
    expect(transactions.length).toBe(0);
    Logger.info(`API correctly handled non-existent account: returned ${transactions.length} transactions`);
  });
  ...
  test('Find Transactions by Amount - Zero Amount', async ({ 
    ...
    if (accounts.length > 0) {
      ...
      const transactions = await api.findTransactionsByAmount(accounts[0].accountId, 0);
      expect(Array.isArray(transactions)).toBeTruthy();
      Logger.info(`Zero amount search returned ${transactions.length} transactions`);
    }
  });
