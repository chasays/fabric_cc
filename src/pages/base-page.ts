...
export abstract class BasePage {
  ...
  async verifyTitleText(
    ...
    const titleLocator = this.page.locator(
      `h1.title:has-text("${expectedText}")`
    );
    await this.verifyElementVisible(titleLocator, description);
  }

  async clickElementWithFallback(
    locator: Locator,
    ...
    try {
      ...
      Logger.info(
        `Primary locator failed, trying fallback for: ${description}`
      );
  ...
  async clickFirstMatchingElement(
    locators: Locator[],
    description: string
  ): Promise<void> {
    ...
    for (let i = 0; i < locators.length; i++) {
      try {
        ...
        Logger.info(
          `Successfully clicked element ${i + 1} for: ${description}`
        );
        return;
      } catch (error) {
        Logger.info(
          `Element ${i + 1} failed for: ${description}, trying next...`
        );
        if (i === locators.length - 1) {
          throw new Error(
            `All ${locators.length} locators failed for: ${description}`
          );
        }
      }
    }
  ...
  async clickWithRetryAndCheck(
    locator: Locator,
    successCheckLocator: Locator,
    description: string,
    maxRetries: number = 3,
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        Logger.info(
          `Click attempt ${attempt}/${maxRetries} for: ${description}`
        );
        await this.waitForElement(locator);
        ...
        if (isSuccessVisible) {
          Logger.info(
            `Click successful - success condition met on attempt ${attempt}`
          );
          return; // success, success check element is visible
        } else {
          Logger.warn(
            `Success condition not met after click on attempt ${attempt}, will retry`
          );
          ...
          if (attempt < maxRetries) {
            ...
            throw new Error(
              `Click failed after ${maxRetries} attempts - success condition never met for ${description}`
            );
          }
        }
      } catch (error) {
        ...
        if (attempt === maxRetries) {
          throw new Error(
            `Click failed after ${maxRetries} attempts for ${description}: ${error}`
          );
        }
        await this.page.waitForTimeout(retryDelay);
      }
