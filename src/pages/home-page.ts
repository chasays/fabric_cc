...
export class HomePage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.productsLink = page.locator(
      '#headerPanel a[href="http://www.parasoft.com/jsp/pr/contacts.jsp"]'
    );
    this.locationsLink = page.locator(
      '#headerPanel a[href="http://www.parasoft.com/jsp/pr/contacts.jsp"]'
    );
    ...
    this.openNewAccountLink = page.locator(
      '#leftPanel a[href="openaccount.htm"]'
    );
    this.accountsOverviewLink = page.locator(
      '#leftPanel a[href="overview.htm"]'
    );
    ...
    this.findTransactionsLink = page.locator(
      '#leftPanel a[href="findtrans.htm"]'
    );
    this.updateContactInfoLink = page.locator(
      '#leftPanel a[href="updateprofile.htm"]'
    );
  ...
  async verifyGlobalNavigationMenuItems(): Promise<void> {
    ...
    for (const item of expectedMenuItems) {
      const menuItemLocator = this.page.locator(
        `#headerPanel a:has-text("${item}")`
      );
      await this.verifyElementVisible(menuItemLocator, `${item} menu item`);
    }

    // verify Solutions item (not li)
    const solutionsLocator = this.page.locator(
      '#headerPanel li.Solutions:has-text("Solutions")'
    );
