...
export class RegistrationPage extends BasePage {
  ...
  async registerUser(user: User): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        Logger.info(
          `Registration attempt ${attempt}/${maxRetries} for user: ${user.username}`
        );
        ...
        if (isUsernameError) {
          Logger.warn(
            `Username ${user.username} already exists, generating new username`
          );
          ...
          if (attempt < maxRetries) {
            ...
            throw new Error(
              `Registration failed after ${maxRetries} attempts - username conflict`
            );
          }
        } else {
          ...
          try {
            ...
            Logger.error(
              `Registration verification failed on attempt ${attempt}: ${error}`
            );
            if (attempt === maxRetries) {
              throw error;
            }
        ...
        Logger.error(
          `Registration attempt ${attempt} failed with error: ${error}`
        );
        if (attempt === maxRetries) {
          throw new Error(
            `Registration failed after ${maxRetries} attempts: ${error}`
          );
        }
        await this.page.waitForTimeout(retryDelay);
      }
  ...
  async verifyRegistrationSuccess(): Promise<void> {
    ...
    await this.verifyText(
      this.successMessageMore,
      'Your account was created successfully. You are now logged in.',
      'Registration success message more'
    );
  }
