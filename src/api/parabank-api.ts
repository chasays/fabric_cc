...
export class ParabankApi {
  ...
  private async getAuthHeaders(): Promise<Record<string, string>> {
    ...
    if (this.page) {
      ...
      Logger.warn(
        'No page context provided, API calls may fail due to missing authentication'
      );
    }
  ...
  async findTransactionsByAmount(
    ...
    if (response.status() === 401) {
      Logger.error(
        'Authentication failed. Please ensure user is logged in before making API calls.'
      );
      throw new Error(
        'API authentication failed. User must be logged in to access this endpoint.'
      );
    }
  ...
  async validateTransactionDetails(
    ...
    Logger.info(
      `Transaction validated: ID=${transaction.id}, Amount=${transaction.amount}, Type=${transaction.type}, Description=${transaction.description}`
    );
  }
}
